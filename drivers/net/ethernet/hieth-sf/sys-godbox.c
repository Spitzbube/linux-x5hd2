...
static void hieth_phy_reset(void)
{
	...
	if (chipid == _HI3712_V100) {
		...
		/* config pin re-use to miirst */
		old = val = readl(HI3716MV300_ETH_GPIO_BASE + HI3716MV300_ETH_GPIO_REG);
		val |= 0x1;
		writel(val, HI3716MV300_ETH_GPIO_BASE + HI3716MV300_ETH_GPIO_REG);
		...
		/* restore pin re-use to old value */
		writel(old, HI3716MV300_ETH_GPIO_BASE + HI3716MV300_ETH_GPIO_REG);
		...
		val = readb(IO_ADDRESS(CONFIG_HIETH_RESET_HELPER_GPIO_BASE) + 0x400);
		val |= (1<< CONFIG_HIETH_RESET_HELPER_GPIO_BIT);
		writeb(val,IO_ADDRESS(CONFIG_HIETH_RESET_HELPER_GPIO_BASE) + 0x400);
