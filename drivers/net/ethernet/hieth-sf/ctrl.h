...
#define hw_recv_cnt_inuse(ld) hieth_readl_bits(ld, UD_REG_NAME(GLB_RO_QUEUE_STAT), BITS_RECVQ_CNT_RXOK)

#define hw_xmitq_pkg(ld, addr, len) do{ \
			hieth_writel(ld, (addr), UD_REG_NAME(GLB_EQ_ADDR)); \
			hieth_writel_bits(ld, (len), UD_REG_NAME(GLB_EQFRM_LEN), BITS_TXINQ_LEN); \
		}while(0)
