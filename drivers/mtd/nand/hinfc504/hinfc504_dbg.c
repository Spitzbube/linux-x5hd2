...
#if defined(CONFIG_HINFC504_DBG_NAND_PROC_FILE) || defined(CONFIG_HINFC504_DBG_NAND_EC_NOTICE)
#  define DBG_NAND_CORRECT_ERROR
#endif /* defined(CONFIG_HINFC504_DBG_NAND_PROC_FILE) || defined(CONFIG_HINFC504_DBG_NAND_EC_NOTICE) */
...
#  ifdef CONFIG_HINFC504_DBG_NAND_EC_NOTICE
	printk("NAND debug: CONFIG_HINFC504_DBG_NAND_EC_NOTICE\n");
...
int dbg_nand_proc_init(void)
{
	...
	printk("NAND debug: CONFIG_HINFC504_DBG_NAND_PROC_FILE\n");
...
static struct gdb_nand_pe_root_t *gdb_nand_pe_root = NULL;
	...
	*eof = 1;
	...
	for (index = nand_pe_root->index;
		...
		ppage += sprintf(ppage,
			...
			index,
			pe[0].count, pe[1].count, pe[2].count, pe[3].count,
			pe[4].count, pe[5].count, pe[6].count, pe[7].count);
	}
		...
		case 'c':
			if (!memcmp(&buf[pos], CMD_WORD_CLEAN,
				...
				memset(nand_pe_root->pe, 0,
					nand_pe_root->num_of_blocks * sizeof(struct gdb_nand_pe_t));
			  ...
			  unsigned int pagesize)
{
	...
	printk("NAND debug: CONFIG_HINFC504_DBG_NAND_PE_PROC_FILE\n");
