...
#ifndef CONFIG_HINFC504_DBG_READ_RETRY_NUM
#  define CONFIG_HINFC504_DBG_READ_RETRY_NUM          (100)
#endif /* CONFIG_HINFC504_DBG_READ_RETRY_NUM */

struct hinfc504_dbg_read_retry_item_t {
	unsigned int page;
...
struct hinfc504_dbg_read_retry_t {
	...
	struct hinfc504_dbg_read_retry_item_t item[CONFIG_HINFC504_DBG_READ_RETRY_NUM];
};

static DEFINE_MUTEX(dbg_read_retry_mutex);
static struct hinfc504_dbg_read_retry_t *dbg_read_retry = NULL;
/*****************************************************************************/
				    ...
				    size_t count, loff_t *ppos)
{
	...
	struct hinfc504_dbg_read_retry_item_t *item;

	if (*ppos == 0) {

		if (dbg_read_retry->count
		    < CONFIG_HINFC504_DBG_READ_RETRY_NUM)
	...
	for (read_index = dbg_read_retry->read_index;
		...
		if (read_index >= CONFIG_HINFC504_DBG_READ_RETRY_NUM)
			read_index = 0;
...
static int dbgfs_read_retry_init(struct dentry *root, struct hinfc_host *host)
{
	...
	struct hinfc504_dbg_read_retry_t *read_retry;
	...
	read_retry = vmalloc(sizeof(struct hinfc504_dbg_read_retry_t));
	if (!read_retry) {
		PR_ERR("Can't allocate memory.\n");
		return -ENOMEM;
	}
	memset(read_retry, 0, sizeof(struct hinfc504_dbg_read_retry_t));
...
static void hinfc504_dbg_read_retry_rr(struct hinfc_host *host, int index)
{
	unsigned int page;
	struct hinfc504_dbg_read_retry_item_t *item;
...
struct hinfc504_dbg_inf_t hinfc504_dbg_inf_read_retry = {
	"read_retry", 0,
